...
func (b *BinlogSyncer) enableSemiSync() error {
	...
	log.Debug("enableSemiSync121++++++++++++++++++++++")
	}

	log.Debug("enableSemiSyn2++++++++++++++++++++++")
	_, err := b.c.Execute(`SET @rpl_semi_sync_slave = 1;`)
	if err != nil {
		return errors.Trace(err)
	}

	log.Debug("OUT enableSemiSync++++++++++++++++++++++")
	return nil
}

func (b *BinlogSyncer) prepare() error {
	log.Debug("ENTER prepare++++++++++++++++++++++")
	if b.isClosed() {
...
func (b *BinlogSyncer) writeRegisterSlaveCommand() error {
	log.Debug("ENTER write rgister slave command++++++++++++++++++++++")
	b.c.ResetSequence()
	...
	log.Debug("OUT write rgister slave command++++++++++++++++++++++")
	return b.c.WritePacket(data)
}

func (b *BinlogSyncer) replySemiSyncACK(p Position) error {
	log.Debug("ENTER replySemiSyncACK++++++++++++++++++++++")
	b.c.ResetSequence()

	log.Debug("1 replySemiSyncACK++++++++++++++++++++++")
	...
	log.Debug("2 replySemiSyncACK++++++++++++++++++++++")
	binary.LittleEndian.PutUint64(data[pos:], uint64(p.Pos))
	pos += 8

	log.Debug("3 replySemiSyncACK++++++++++++++++++++++")
	copy(data[pos:], p.Name)

	log.Debug("4 replySemiSyncACK++++++++++++++++++++++")
	err := b.c.WritePacket(data)
	if err != nil {
		return errors.Trace(err)
	}

	log.Debug("OUT replySemiSyncACK++++++++++++++++++++++")
	return nil
}
...
func (b *BinlogSyncer) prepareSyncPos(pos Position) error {
        log.Debug("ENTER prepareSyncPos+++++++++++++++++++++++++")
	// always start from position 4
	if pos.Pos < 4 {
		pos.Pos = 4
	}

        log.Debug("1 prepareSyncPos+++++++++++++++++++++++++")
	if err := b.prepare(); err != nil {
		return errors.Trace(err)
	}

        log.Debug("2 prepareSyncPos+++++++++++++++++++++++++")
	if err := b.writeBinlogDumpCommand(pos); err != nil {
		return errors.Trace(err)
	}

        log.Debug("OUT prepareSyncPos+++++++++++++++++++++++++")
	return nil
}
...
func (b *BinlogSyncer) onStream(s *BinlogStreamer) {
	...
	for {
		...
		if err != nil {
			...
			for {
                                log.Debug("start try resync++++++++++++++++")
				select {
				case <-b.ctx.Done():
					s.close()
					return
				case <-time.After(time.Second):
                                        log.Debug("try resync after 1 second ++++++++++++++++")
			...
			continue
                ...
                log.Debug("2  read packet++++++++++++++++")
		//set read timeout
		if b.cfg.ReadTimeout > 0 {
			b.c.SetReadDeadline(time.Now().Add(b.cfg.ReadTimeout))
		}

                log.Debug("3  read packet++++++++++++++++")
		switch data[0] {
		case OK_HEADER:
                        log.Debug("OKHEADER  ++++++++++++++++")
